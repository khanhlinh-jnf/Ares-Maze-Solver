...
def Convergent_Swarm(file_path):
	...
	pr_queue.put((init_heuristic, ares_position, frozenset(stone_positions), "", 0, stones_value, 0)) 
	explored = set() 
	
	while not pr_queue.empty(): 
		_, (ax, ay), stones, path, cost, value, current  = pr_queue.get()
		...
		for next_x, next_y, move in [(-1, 0, 'u'), (0, -1, 'l'), (0, 1, 'r'), (1, 0, 'd')]:
			...
			if not is_deadlock: pr_queue.put((g + 100 * new_heuristic,(new_x, new_y), frozenset(new_stones), new_path, new_cost, new_stones_value, g))
...
if __name__ == "__main__":
	...
	# 	print(Convergent_Swarm(file_path))
